        ...
        do
        {
            ...
            for (auto clienteIt = std::next(veiculo->listaClientes.begin(), 1); clienteIt != veiculo->listaClientes.end();)
            {
                ...
                if (!passouEscolhido)
                {
                    ...
                    if(hashRotas)
                        resultado = hashRotas->getVeiculo(vetClienteRotaBest, posicaoAux, veiculo->tipo, &poluicao, &combustivel);
                        ...
                        if (modelo)
                        {
                            ...
                            if (modelo->usaModeloVnd)
                            {
                                ...
                                resultado = modelo->criaRota(vetClienteRotaBest, posicaoAux, veiculo->tipo, peso,
                                                             instancia, &poluicao, &combustivel);
...
/*    while(true)
    ...
    do
    {
        ...
        for (auto clienteIt = std::next(veiculo1->listaClientes.begin(), 1); clienteIt != veiculo1->listaClientes.end();)
        {
            ...
            {
                if(hashRotas)
                    resultado = hashRotas->getVeiculo(vetClienteRotaBest, posicaoAux, veiculo1->tipo, &poluicaoAuxVeic1, &combustivelAuxVeic1);
                    ...
                    if (modelo)
                    {
                        if (modelo->usaModeloVnd)
                        {
                            resultado = modelo->criaRota(vetClienteRotaBest, posicaoAux, veiculo1->tipo, PesoVeiculo1,
                                                         instancia, &poluicaoAuxVeic1, &combustivelAuxVeic1);
                        } else
                        {
                            resultado = Movimentos_Paradas::criaRota(instancia, vetClienteRotaBest, posicaoAux,
                    ...
                    if (clientePtr->cliente == (*clienteEscolhido)->cliente)
                    {
                        ...
                        if(veiculo2->tipo != 2)
                        {
                            if(posicaoAux > 2)
                            {
                                ...
                                if(hashRotas)
                                    resultado = hashRotas->getVeiculo(vetClienteRotaSecundBest, posicaoAux, veiculo2->tipo, &poluicaoRotaVeic2, &combustivelRotaVeic2);
                                    ...
                                    if (modelo)
                                    {
                                        ...
                                        } else
                                        {
                                            resultadoVeic2 = Movimentos_Paradas::criaRota(instancia,
                                                                                          ...
                                                                                          posicaoAux, PesoVeiculo2,
                                                                                          veiculo2->tipo,
                                                                                          &combustivelRotaVeic2,
                                                                                          &poluicaoRotaVeic2, NULL,
                                                                                          NULL,
                                                                                          vetLimiteTempo,
                                                                                          vetClienteRotaAux);
                                        }
                                            ...
                                            HashRotas::HashRotas *hashRotas)
{
    ...
    int posicaoClienteEscolhido;
    auto clienteEscolhido = std::next(veiculo2->listaClientes.begin(), 1);
    ...
    bool inicioRota = true;
    int demandaEscolhido;

    const int Veiculo1PosicaoOriginal = veiculoEscolhido1;
    vetClienteRotaBest[0].cliente = vetClienteRotaSecundBest[0].cliente = vetClienteRotaSecundBest[0].cliente = vetClienteRotaSecundAux[0].cliente = 0;
    ...
    do
    {
        ...
        // posicaoClienteEscolhido, clienteEscolhido, demandaEscolhido
        posicaoClienteEscolhido = 0;

        for(clienteEscolhido = std::next(veiculo2->listaClientes.begin(), 1); (*clienteEscolhido)->cliente != 0; ++clienteEscolhido)
        {
            posicaoClienteEscolhido += 1;
            demandaEscolhido = instancia->vetorClientes[(*clienteEscolhido)->cliente].demanda;
            ...
            for (auto clienteIt = std::next(veiculo1->listaClientes.begin(), 1); clienteIt != veiculo1->listaClientes.end();)
            {
                ...
                if (PesoVeic1T < instancia->vetorVeiculos[veiculo1->tipo].capacidade)
                {
                    ...
                    if (PesoVeic2T < instancia->vetorVeiculos[veiculo2->tipo].capacidade)
                    {
                        ...
                        if(hashRotas)
                            resultadosRotaVeic1 = hashRotas->getVeiculo(vetClienteRotaBest, posicaoAuxVeic1, veiculo1->tipo, &poluicaoAuxVeic1, &combustivelAuxVeic1);
                        ...
                        if (resultadosRotaVeic1)
                        {
                            ...
                            if (veiculo2->tipo != 2)
                            {
                                ...
                                if(hashRotas)
                                    resultadosRotaVeic2 = hashRotas->getVeiculo(vetClienteRotaSecundBest, posicaoAuxVeic2, veiculo2->tipo, &poluicaoAuxVeic2, &combustivelAuxVeic2);
                            ...
                            {   //cout<<"Gerou solucao\n";
                                ...
                                if (veiculo2->tipo != 2)
                                    viavel = (poluicaoNovaSolucao < (veiculo1->poluicao + veiculo2->poluicao)) ||
                                             pertubacao;
                                else
                                    (poluicaoNovaSolucao <
                                     (veiculo1->poluicao + veiculo2->poluicao + solucao->poluicaoPenalidades)) ||
                                    pertubacao;
            ...
            vetClienteRotaSecundBest[posicaoClienteEscolhido].cliente = (*clienteEscolhido)->cliente;
        }
...
/*
 * Escolher uma rota. Percorrer as outras aresta e invertendo os clientes entre as duas arestas. As arestas não podem ser do mesmo cliente.
 * OK
 */
                                                ...
                                                Modelo::Modelo *modelo, HashRotas::HashRotas *hashRotas)
{
    ...
    do
    {
        ...
        posicaoClienteEscolhido = 0;
        clienteEscolhido = std::next(veiculo->listaClientes.begin(), 1);

        for(; (*clienteEscolhido)->cliente != 0; ++clienteEscolhido)
        {
            ...
            for (auto clienteIt = std::next(veiculo->listaClientes.begin(), 1); clienteIt != veiculo->listaClientes.end();)
            {
                ...
                //Cliente escolhido é clienteIt.  duas arestas do mesmo cliente
...
/*            cout<<"clientePassouEscolhido "<<clientePassouEscolhido;
            ...
            cout<<"\n\n***************************************************************\n\n";*/
                ...
                if(hashRotas)
                    resultadosRota = hashRotas->getVeiculo(vetClienteRotaBest, posicaoAux, veiculo->tipo, &poluicaoAux, &combustivelAux);
...
//clienteEscolhido, posicaoClienteEscolhido

    //Escolhe um cliente
    int posicaoClienteEscolhido = rand_u32() % (veiculo2->listaClientes.size() - 2);
    ...
    auto itAux_ = veiculo2->listaClientes.begin();
    vetClienteRotaSecundBest[0] = **itAux_;
...
/*    for (auto it : veiculo2->listaClientes)
    ...
    do
    {
        ...
        posicaoClienteEscolhido = 0;
        auto ultimoVeic2 = veiculo2->listaClientes.end();
        ultimoVeic2--;          //0
        ultimoVeic2--;          //x

        for(clienteEscolhido = std::next(veiculo2->listaClientes.begin(), 1); (*clienteEscolhido)->cliente != (*ultimoVeic2)->cliente; ++clienteEscolhido)
        {
                ...
                if (PesoNovoVeiculo1 < instancia->vetorVeiculos[veiculo1->tipo].capacidade)
                {
                    ...
                    if (PesoNovoVeiculo2 < instancia->vetorVeiculos[veiculo2->tipo].capacidade)
                    {
                        ...
                        if (posicaoAuxVeic1 > 2)
                        {
                            ...
                            if(hashRotas)
                                resultadosRotaVeic1 = hashRotas->getVeiculo(vetClienteRotaBest, posicaoAuxVeic1, veiculo1->tipo, &poluicaoVeic1, &combustivelVei1);
                        ...
                        if (resultadosRotaVeic1)
                        {
                            ...
                            bool resultadosRotaVeic2 = false;

                            if (posicaoAuxVeic2 > 2)
                            {

                                if(hashRotas)
                                    resultadosRotaVeic2 = hashRotas->getVeiculo(vetClienteRotaSecundBest, posicaoAuxVeic2, veiculo2->tipo, &poluicaoVeic2, &combustivelVeic2);
                            ...
                            if (resultadosRotaVeic2)
                            {
                                ...
                                if (((poluicaoVeic1 + poluicaoVeic2) < (veiculo1->poluicao + veiculo2->poluicao)) ||
                                    pertubacao)
                                {
                                   ...
                                   HashRotas::HashRotas *hashRotas)
{
    ...
    for (int i = 0; i < numVeic; ++i)
    {
        ...
        if(hashRotas)
            resultadosRota = hashRotas->getVeiculo(vetClienteRotaBest, posicao, veiculo->tipo, &poluicao, &combustivel);
    ...
    while (!resultadosRota)
    {
        ...
        if (veiculo1->listaClientes.size() > 2)
        {
            ...
            if (veiculo1->carga < instancia->vetorVeiculos[novoTipo].capacidade)
            {
                ...
                if(hashRotas)
                    resultadosRota = hashRotas->getVeiculo(vetClienteRotaBest, posicaoVeic1, novoTipo, &poluicaoVeic1, &combustivelVeic1);
                            ...
                            double *vetLimiteTempo, Modelo::Modelo *modelo, HashRotas::HashRotas *hashRotas)
{
    ...
    Movimentos::ResultadosRota resultado;

    if(vetClienteRotaBest)
        vetClienteRotaBest[0].cliente = 0;

    if(vetClienteRotaAux)
        vetClienteRotaAux[0].cliente = 0;

    if(vetClienteRotaSecundBest)
        vetClienteRotaSecundBest[0].cliente = 0;

    if(vetClienteRotaSecundAux)
        vetClienteRotaSecundAux[0].cliente = 0;
    ...
    switch (movimento)
    {
        ...
        case 1:
            resultado = Movimentos::mvIntraRotaSwap(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux, true,
                                                    true,
                                                    pertubacao, vetLimiteTempo, modelo, hashRotas);
            break;

        case 2:
            resultado = Movimentos::mvInterRotasShift(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                      vetClienteRotaSecundBest, true, pertubacao, vetLimiteTempo,
                                                      modelo, hashRotas);
            break;
        case 3:
            resultado = Movimentos::mvInterRotasSwap(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                     vetClienteRotaSecundBest, vetClienteRotaSecundAux, true,
                                                     pertubacao,
                                                     vetLimiteTempo, modelo, hashRotas);
            break;

        case 4:
            resultado = Movimentos::mv_2optSwapIntraRota(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                         true,
                                                         pertubacao, vetLimiteTempo, modelo, hashRotas);
            break;

        case 5:
            resultado = Movimentos::mv_2optSwapInterRotas(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                          vetClienteRotaSecundBest, vetClienteRotaSecundAux, true,
                                                          pertubacao, vetLimiteTempo, modelo, hashRotas);
            break;

        case 6:
            resultado = Movimentos::mvIntraRotaInverteRota(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                           pertubacao, vetLimiteTempo, modelo, hashRotas);
            break;

        case 7:
            resultado = Movimentos::mvTrocarVeiculos(instancia, solucao, vetClienteRotaBest, vetClienteRotaAux,
                                                     vetClienteRotaSecundBest, vetClienteRotaSecundAux, pertubacao,
                                                     vetLimiteTempo, modelo, hashRotas);
            break;
    }
