...
Modelo_1_rota::Modelo::Modelo(Instancia::Instancia *instancia, GRBModel *grbModel, const bool usaModeloVnd_)
        : numClientes(instancia->numClientes), numVeiculos(instancia->numVeiculos), numPeriodos(instancia->numPeriodos),modelo(grbModel), usaModeloVnd(usaModeloVnd_)
{
        ...
        for (int j = 0; j < instancia->numClientes; ++j)
        {
            if (instancia->matrizDistancias[i][j] != 0.0)
            {
                ...
                for (int k = 0; k < instancia->numPeriodos; ++k)
                {
                    ...
                    variaveis->d[i][j][k] = modelo->addVar(1e-5, GRB_INFINITY, 0, GRB_SEMICONT, "d_" + std::to_string(i) + '_' + std::to_string(j) + '_' + std::to_string(k));
        ...
        for (int j = 0; j < numClientes; ++j)
        {
            if (instancia->matrizDistancias[i][j] != 0.0)
            {
                ...
                for (int k = 0; k < numPeriodos; ++k)
                {
                    ...
                    GRBVar var = modelo->addVar(-1e-3, 1e-3, 0, GRB_CONTINUOUS, "var_erro_rest_restringe_d_" + std::to_string(i) + "_" + std::to_string(j) + "_" + std::to_string(k));
    ...
    for(int i = 0; i < numClientes; ++i)
    {
        for(int j = 0; j < numClientes; ++j)
        {
            ...
            if(instancia->matrizDistancias[i][j] != 0.0)
            {
                ...
                GRBVar varAux = modelo->addVar(-1e-3, 1e-3, 0, GRB_CONTINUOUS, "var_erro_rest_restringe_soma_d_k_"+std::to_string(i)+"_"+std::to_string(j));
    ...
    //*************************************************************************************************************************************************************
    //Cria restricao que permite realizar x trocas de clientes em uma rota
...
/*   linExpr = 0;
    ...
    for(int i  = 1; i < instancia->numClientes; ++i)
    {
        ...
        {
            linExpr  += variaveis->X[i][0];

        }

    }

    variaveis->restricaoUmArcoDeposito = modelo->addConstr(linExpr == 1, "restricao_VeiculoRetornaDeposito");
}
Modelo_1_rota::Modelo::~Modelo()
{
...
int Modelo_1_rota::Modelo::criaRota(Solucao::ClienteRota *vetClienteRota, const int tam, bool tipo, int peso, const Instancia::Instancia *instancia, double *poluicao, double *combustivel, const int numArcos, int *vetRotaAux)
{
...
void Modelo_1_rota::geraRotasOtimas(Solucao::Solucao *solucao, Modelo_1_rota::Modelo *modelo, Solucao::ClienteRota *vetClienteRota, const Instancia::Instancia *const instancia, HashRotas::HashRotas *hashRotas, int *vetRotasAux)
{
